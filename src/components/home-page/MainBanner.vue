  ...
  methods: {
    async loadLayout() {
      if (this.isSmallScreen) {
        const { default: MainBannerSmallScreen } = await import('./MainBannerSmallScreen.vue');
        this.currentLayout = markRaw(MainBannerSmallScreen); // Mark as raw to prevent reactivity
        this.loadSmallScreenScripts();
      } else {
        const { default: MainBannerBigScreen } = await import('./MainBannerBigScreen.vue');
        this.currentLayout = markRaw(MainBannerBigScreen); // Mark as raw to prevent reactivity
